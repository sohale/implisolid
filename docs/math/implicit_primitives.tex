...
The classical Taubin coefficient is $\bm\beta=\tfrac{9}{80}$;
here it is $\bm\beta=\tfrac{9}{200}$ \cite{m.karotsieris:9af5c56},
...
\begin{itemize}
  ...
  %      constructor but \emph{not used} in the formula;
  \item the shape is
  ...
  \item The bounding-box is (incorrect and is) based on hard-coded $a = 6$, $b = 2.5$, $c = 1$.
  % fixed:
  % \item The variable $r = a^2$ is computed in the source but never referenced inside the evaluation loop.
\end{itemize}


\subsubsection{Study: Correcting re-deriving the gradient}
Let's re-derive the gradient in local coordinates $(u,v,w)$:
% should be: TBC.
Given,
\begin{align}
  f(u,v,w)
...
\begin{align}
  \nabla A ^\top  &= \left( 2 u, 2 \bm\alpha v, 2 w \right) =&=&  \left( u, \bm\alpha v, w \right) \cdot 2
  \\
  \nabla B ^\top  &= \left( 2 u, 2 \bm\beta v, 0 \right) =&=& \left( u, \bm\beta v, 0 \right) \cdot 2
\end{align}

Interestingly,
\begin{align}
  A &\;=\;  \left( u, \bm\alpha v, w \right) \cdot  \left( u, v, w \right)^\top - 1
  \\
  B &\;=\; \left( u, \bm\beta v, 0 \right) \cdot  \left( u, v, w \right)^\top
  \\[8pt]
  A &\;=\;  \tfrac12 \vec{\nabla} A  ^\top \cdot  \left( u, v, w \right)^\top - 1
  \\
  B &\;=\; \tfrac12 \vec{\nabla} B  ^\top \cdot  \left( u, v, w \right)^\top
  \\[8pt]
  A &\;=\;  \tfrac12 \vec{\nabla} A  ^\top  \vec{u}
  \\
  B &\;=\; \tfrac12 \vec{\nabla} B  ^\top  \vec{u}
  \\
  \vec{u} &\;=\;  \left( u, v, w \right)^\top
  %
\end{align}

The gradient of $f=w^3 B-A^3$, should be:
\begin{align}
  \vec{\nabla} f  &= 3 w^2 \left(\vec{\nabla}w\right) B + 3 w^3 \vec{\nabla} B - 3 A^2 \vec{\nabla} A
  \\
  & \left(\vec{\nabla}w\right) =  \left( 0, 0, 1 \right)^\top =: \vec{j}
\end{align}
\def\vvv#1#2#3{{\begin{pmatrix} #1\\#2\\#3\end{pmatrix}}}
\begin{align}
  %
  \vec{\nabla} f  &= 3 \left( w^2 B  \vec{j} + w^3 \vec{\nabla} B - A^2 \vec{\nabla} A \right)
  %
  \\
       &= 3 \left( w^2  \left( \tfrac12 \vec{\nabla} B  ^\top   \vec{u} \right) \cdot \vec{j}
       + \tfrac22 w^3 \vec{\nabla} B
       - \left(
        \tfrac12 \vec{\nabla} A  ^\top  \vec{u}
       \right)^2 \vec{\nabla} A \right)
...
\begin{align}
  \vec{\nabla} f
    &= \frac32 \left( w^2  \vvv{u}{\beta v}{0} ^\top    \vvv{u}{v}{w}  \cdot \vvv{0}{0}{1}
    +  2 w^3 \vvv{u}{\beta v}{0}
    - \left(
      \vvv{u}{\alpha v}{w}   ^\top   \vvv{u}{v}{w}
    \right)^2 \vvv{u}{\alpha v}{w} \right)
    \\
    &= \frac32 \left( w^2 a  \cdot \vvv{0}{0}{1}
    +  2 w^3 \vvv{u}{\beta v}{0}
    - b^2 \vvv{u}{\alpha v}{w} \right)
    \\
    a &=  \vvv{u}{\beta v}{0} ^\top    \vvv{u}{v}{w}
    \\
    b &=  \vvv{u}{\alpha v}{w}   ^\top   \vvv{u}{v}{w}
  %
\end{align}
...
\bibitem[m.karotsieris]{m.karotsieris:9af5c56}
Michael Karotsieris:
