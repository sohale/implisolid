...
\subsection{Coordinate transform}\label{common:homogenic-transform}
...
First, it applies the coordinate transform \ref{common:homogenic-transform}.
Then,
...
% Equivalently, the zero set (the surface) satisfies:
\begin{equation}
  \left(u^2 + \tfrac{9}{4}\,v^2 + w^2 - 1\right)^3
  \;=\;
...
The classical Taubin coefficient is $\tfrac{9}{80}$; here it is $\tfrac{9}{200}$,
...
The gradient in local coordinates $(u,v,w)$ is:
\begin{align}
...
\bibitem{schauvier:800358f}[schauvier]
Charles Chauvier:
Heart implicit function and gradient.
\texttt{s.chauvier} \texttt{800358f}.

\bibitem{michael.karotsieris:9af5c56}[m.karotsieris]
Michael Karotsieris:
