...
class half_plane_x_SDF : public transformable_implicit_function {

protected:
    REAL x0;
    int direction; //0 = x0 - x >=0, 1= x - x0 >= 0

public:

    half_plane_x_SDF(REAL matrix12[12], REAL x0, int direction)
    {

      this->x0 = x0;
      this->direction = direction;
      /*if(this->direction != 0 && this->direction != 1){
        throw new std::exception("Wrong direction argument");
      }*/
    ...
    static void getHalfXParameters( REAL& x0,
                    const pt::ptree& shapeparams_dict){

        std::cout << "big thing: " << shapeparams_dict.get<float>("x0") << std::endl;
        x0 = shapeparams_dict.get<float>("x0");
