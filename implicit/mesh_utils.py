...
def make_neighbour_faces_of_vertex(faces):
    #The indices of faces that one vertex belong to. There are 3 or more of them.
    ...
    for fi in range(faces.shape[0]):
        for vi in range(3):
            v1 = faces[fi, vi]
            if not v1 in neighbour_faces_of_vertex:
                neighbour_faces_of_vertex[v1] = [fi]
            else:
                assert not fi in neighbour_faces_of_vertex[v1]
