...
echo "for latest commit info click: try http://localhost:$MYPORT8000/latest-commit-log.txt"
...
export DEPLOY_LOCATION=\"$DEPLOY_LOCATION\"
cd $DEPLOY_LOCATION/
# or if on a different machine:
# scp -r digo:$DEPLOY_LOCATION . && cd demo1

echo visit http://localhost:$MYPORT8000/mp5_json_code.html
python3 -m http.server $MYPORT8000
LAUNCH_SCRIPT
