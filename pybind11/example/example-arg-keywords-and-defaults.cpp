...
void kw_func(int x, int y) { lout << "kw_func(x=" << x << ", y=" << y << ")" << std::endl; }
...
void args_function(py::args args) {
    for (size_t it=0; it<args.size(); ++it)
        lout << "got argument: " << py::object(args[it]) << std::endl;
}

void args_kwargs_function(py::args args, py::kwargs kwargs) {
    for (auto item : args)
        lout << "got argument: " << item << std::endl;
    if (kwargs) {
        for (auto item : kwargs)
            lout << "got keyword argument: " << item.first << " -> " << item.second << std::endl;
    }
}
