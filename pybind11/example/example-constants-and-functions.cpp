...
public:
    ...
    static EMode test_function(EMode mode) {
        lout << "ExampleWithEnum::test_function(enum=" << mode << ")" << std::endl;
        return mode;
    }
};

bool test_function1() {
    lout << "test_function()" << std::endl;
    return false;
}

void test_function2(EMyEnumeration k) {
    lout << "test_function(enum=" << k << ")" << std::endl;
}

float test_function3(int i) {
    lout << "test_function(" << i << ")" << std::endl;
    return (float) i / 2.f;
}
...
void print_bytes(py::bytes bytes) {
    ...
    for (size_t i = 0; i < value.length(); ++i)
        lout << "bytes[" << i << "]=" << (int) value[i] << std::endl;
}
